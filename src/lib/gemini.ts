...
- 件名は不要。本文のみ出力。`
  ...
  // 件名は「前半（お礼の文言）」だけAIに作らせ、末尾の差出人表記はコードで強制連結する。
  // これによりGemini側の指示無視（受信者情報を末尾に流用する事故）を構造的に防止。
  const SUBJECT_SUFFIX = '（株式会社ライフタイムサポート 龍竹）'
  const subjectPrompt = `あなたはお礼メールの件名の【前半部分】のみを作成する担当です。

【出力ルール】
- 「○○のお礼」または「○○の御礼」の形式で、件名の前半部分のみを1行で出力してください
...
出会いの文脈（episodeMemo・先頭150文字）: ${episodeMemo?.slice(0, 150) || '（不明）'}`
  ...
  // 万が一空文字になった場合のフォールバック
  const safePrefix = subjectPrefix || '名刺交換のお礼'
