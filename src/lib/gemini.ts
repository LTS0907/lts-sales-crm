...
- 件名は不要。本文のみ出力。`
  ...
  const subjectPrompt = `あなたは株式会社ライフタイムサポート 代表取締役 龍竹一生 が送るお礼メールの件名作成担当です。
以下のルールを厳守して件名を1行だけ出力してください。説明・前置き・引用符・「件名:」の接頭辞は一切不要です。

【件名の必須フォーマット】
{接点・出来事のお礼}（株式会社ライフタイムサポート 龍竹）

【ルール】
- 全角30文字以内（半角換算50文字以内）に収めること
- 末尾は必ず「（株式会社ライフタイムサポート 龍竹）」とし、誰からのメールか明示すること
- 出会いの文脈（イベント名・展示会名・紹介者・場所・場面）が分かる場合は冒頭に入れる
- 文脈が不明な場合は「名刺交換のお礼」「先日のご挨拶のお礼」など汎用文言を使う
- 「○○のお礼」「○○の御礼」を必ず含める（お礼メールであることを明示）

【絶対にやってはいけないこと】
- ❌ 受信者の氏名・会社名・役職を件名に入れる（例: 「栗原様 社労士事務所」）— 宛名は本文側で書く
- ❌ 売り込み感のある言葉（「ご提案」「ご商談」「サービスのご案内」「打ち合わせ」等）
- ❌ 抽象的すぎる件名（「ご連絡」「お知らせ」「初めまして」のみ等）
- ❌ 末尾の差出人表記を省略する

【良い例】
✅ 名刺交換のお礼（株式会社ライフタイムサポート 龍竹）
✅ 先日の2月例会にてご挨拶の御礼（株式会社ライフタイムサポート 龍竹）
✅ ◯◯展示会にて名刺交換のお礼（株式会社ライフタイムサポート 龍竹）
✅ ご紹介いただきましたお礼（株式会社ライフタイムサポート 龍竹）

【悪い例】（直近の生成失敗ケース）
❌ 栗原武士 社会保険労務士事務所 栗原武士 様 ← 宛名そのまま
❌ 営業のご提案 ← 売り込み感
❌ サービスのご案内 ← 売り込み感

【参考情報】
出会いの文脈（episodeMemo・先頭150文字）: ${episodeMemo?.slice(0, 150) || '（不明）'}
会社名: ${company || ''}（※件名には入れない・参考のみ）`
