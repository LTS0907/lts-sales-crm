...
export async function POST(request: NextRequest) {
  ...
  const contact = await prisma.contact.create({
    data: { id: data.id || crypto.randomUUID(), updatedAt: new Date(), name: data.name, nameKana: data.nameKana, connectionType: data.connectionType || null, company: data.company, department: data.department, title: data.title, email: data.email, phone: data.phone, lineId: data.lineId, gmailAlias: data.gmailAlias, website: data.website, address: data.address, episodeMemo: data.episodeMemo, owner: data.owner || 'KAZUI' },
  })
